...
import { View, Text, StyleSheet, Dimensions, Modal, TouchableOpacity, TextInput } from 'react-native';
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabView}>
        <TouchableOpacity
          style={[styles.tabs, { borderColor: connectionTab ? ColorSet.defaultTheme : ColorSet.textBlack }]}
          onPress={() => tabsChangingHandler(TabType.connections)}>
          <Text style={[styles.tabsText, { color: connectionTab ? ColorSet.defaultTheme : ColorSet.textBlack }]}>
            Connections
          </Text>
          <Image style={styles.dotIcon} source={ImageSet.dot} />
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.tabs, { borderColor: requestTab ? ColorSet.defaultTheme : ColorSet.textBlack }]}
          onPress={() => tabsChangingHandler(TabType.requests)}>
          <Text style={[styles.tabsText, { color: requestTab ? ColorSet.defaultTheme : ColorSet.textBlack }]}>
            Requests
          </Text>
          <Image style={styles.dotIcon} source={ImageSet.dot} />
        </TouchableOpacity>
        ...
        connectionsArray.length > 0 ?
          <View style={{ flex: 1, paddingVertical: 20, paddingHorizontal: 20 }}>
            {connectionsArray.map((item, index) => {
              return (
                <SkeletonContent
                  containerStyle={{ flexDirection: 'row', alignItems: 'center', height: 40, width: screenWidth.width75, margin: 10 }}
                  isLoading={true}
                  isVisible={true}
                  // boneColor="#ededed"
                  ...
                  layout={[
                    // long line
                    { width: 45, height:45, resizeMode:'contain', borderRadius: 150, marginBottom: 10},
                    { width: screenWidth.width65, height: 20, marginBottom: 10, marginLeft: 10 },
                    { width: 25, height:25, borderRadius: 10, marginBottom: 10, marginLeft: 10 },
                  ]}
                >
                <TouchableOpacity key={index} style={styles.connectionsView}>
                  ...
                  {item.count &&
                    ...
                    ||
                    <View style={{ width: screenWidth.width5 + 20 }} />
                  }
                </TouchableOpacity>
                </SkeletonContent>
              )
