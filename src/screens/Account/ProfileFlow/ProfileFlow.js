...
export default function ProfileFlow({ navigation }) {
  ...
  const _retriveUser = async () => {
    ...
    let temp = pronounsList.findIndex((x) => {
      return x.label == currUser.pronouns
    })
    let temp2 = ethnicityList.findIndex((x) => {
      return x.label == currUser.ethnicity
    })
  ...
  const setIceBreaker = (value) => {
    ...
    if (
      ...
      newUser.iceBreaker !== null &&
      newUser.interests !== null &&
      newUser.pronouns !== null &&
      newUser.profileImages.length > 0
    ) {
          ...
          {user === null ? (
            ...
            <>
              ...
              {step === 1 ? (
                <AddPhotos
                  user={user}
                  backHandler={cancelHandler}
                  nextHandler={nextHandler}
                  actionHandler={setImages}
                />
              ) : null}
              {step === 2 ? (
                <AddPronouns
                  initialValue={pronounValue}
                  radioProps={pronounsList}
                  user={user}
                  backHandler={backHandler}
                  nextHandler={nextHandler}
                  actionHandler={setPronouns}
                />
              ) : null}
              {step === 3 ? (
                <AddEthnicity
                  initialValue={ethnicityValue}
                  radioProps={ethnicityList}
                  user={user}
                  backHandler={backHandler}
                  nextHandler={nextHandler}
                  actionHandler={setEthnicity}
                />
              ) : null}
              {step === 4 ? (
                <AddIceBreaker
